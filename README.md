...
- Controlador de torneig amb càmera o imatge QR, detecció de noms repetits, revisió d’identitats, classificació acumulada, fusió de duplicats i còpies JSON.
...
Els noms, la partida en curs i el torneig es desen a `localStorage` del dispositiu. No hi ha analítica, servidor ni transmissió automàtica. En finalitzar una partida, el QR incorpora el nom de la taula, els noms dels participants, els resultats i, si s’ha informat per desfer un empat, els diners que porten a sobre; mostrar o compartir aquest QR és una decisió explícita de l’usuari.
